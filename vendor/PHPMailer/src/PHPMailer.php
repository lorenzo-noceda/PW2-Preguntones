...
/**
 * MailPresenter - PHP email creation and transport class.
 * PHP Version 5.5.
 *
 * @see https://github.com/PHPMailer/PHPMailer/ The MailPresenter GitHub project
 *
...
class PHPMailer
{
     ...
     * If empty, MailPresenter attempts to find one with, in order,
     ...
     * Default is $Hostname. If $Hostname is empty, MailPresenter attempts to find
    ...
    /**
     * An implementation of the MailPresenter OAuthTokenProvider interface.
     *
     ...
     * By default MailPresenter will use `echo` if run from a `cli` or `cli-server` SAPI, `html` otherwise.
     ...
     * @var bool
     *
     * @deprecated 6.0.0 MailPresenter isn't a mailing list manager!
     */
     ...
     * Options: An empty string for MailPresenter default, whitespace/null for none, or a string to use.
     *
     * @var string|null
     */
    public $XMailer = '';
    ...
    /**
     * The MailPresenter Version number.
     *
     * @var string
     */
    const VERSION = '6.9.2';
    ...
    /**
     * Add an address to one of the recipient arrays or to the ReplyTo array. Because MailPresenter
     * can't validate addresses with an IDN without knowing the MailPresenter::$CharSet (that can still
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        //Enqueue addresses with IDN until we know the MailPresenter::$CharSet.
     ...
     *
     * ```php
     * MailPresenter::validateAddress('user@example.com', function($address) {
     *     return (strpos($address, '@') !== false);
     * });
     * ```
     *
     * You can also set the MailPresenter::$validator static to a callable, allowing built-in methods to use your validator.
     *
     ...
     * Important: Address must be passed in same encoding as currently set in MailPresenter::$CharSet.
    ...
    public function createHeader()
    {
        ...
        if ('' === $this->XMailer) {
            ...
            $result .= $this->headerLine(
                'X-Mailer',
                'MailPresenter ' . self::VERSION . ' (https://github.com/MailPresenter/MailPresenter)'
            );
     ...
     * MailPresenter only supports some preset message types, not arbitrary MIME structures.
     */
     ...
     * Explicitly *does not* support passing URLs; MailPresenter is not an HTTP client.
     ...
     *                            MailPresenter will attempt to set a MIME type for the attachment.
    ...
    protected function lang($key)
    {
        ...
        if (array_key_exists($key, $this->language)) {
            if ('smtp_connect_failed' === $key) {
                ...
                //but it's usually not MailPresenter's fault.
                return $this->language[$key] . ' https://github.com/MailPresenter/MailPresenter/wiki/Troubleshooting';
            }
